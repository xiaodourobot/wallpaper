...
function addJigsaw(img, alt){
    ...
    if(jigsaw.halfHtml !== '')    //  1/4 的壁纸，已加载完上面一半，接着加载下面那半
    {
        ...
        newHtml = '    <div class="Hhalf oneImg" onmouseover="hoverJigsaw(this)">'
                + '        <img data-original="https://bz.xiaodoubi.com/img.php?url=' + decode360Url(img, imgWidth, imgHeight, 0) + '" alt="' + alt + '" title="关键字：' + alt + '" data-realurl="https://bz.xiaodoubi.com/img.php?url=' + img + '">'
                + '    </div>'
                + '</div>';
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            for (var i = 0; i < jsonData.images.length; i++){
                ...
                newHtml += '<section data-url="' + downUrl + '" data-img="https://bz.xiaodoubi.com/img.php?url=http://cn.bing.com' + jsonData.images[i].url + '"><p class="note">' + jsonData.images[i].copyright + '</p></section>';
            }
